...
public class Main {
    public static void main(String[] args) {
        ...
        List<Melon> europeansLambda = Filters.filterMelons(melons, melon -> "europe".equalsIgnoreCase(melon.getOrigin()));
        System.out.println("Europeans (via lambda): "+europeansLambda);

        List<Melon> waterMelons = Filters.filter(melons, (Melon m) -> "Watemelon".equalsIgnoreCase(m.getType()));
        System.out.println("Watermelons: "+waterMelons);

        List<Integer> numbers = Arrays.asList(1,13,15,2,67);
        List<Integer> smallerThan10 = Filters.filter(numbers, (Integer i) -> i<10);
        System.out.println("Numbers < 10: " + smallerThan10);
    }
}
